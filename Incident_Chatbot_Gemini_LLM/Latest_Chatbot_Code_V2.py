...
embed_model = lambda text: embed_content(model="models/embedding-001", content=text)['embedding']

# Initialize LangChain LLM and Tools
llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash")
retrieval_tool = Tool(
    name="Document Search",
    func=lambda query: retrieve_document(query) or "No relevant document found.",
    description="Searches indexed Excel documents for relevant information."
)
        ...
        if file.endswith(".xlsx") or file.endswith(".xls"):
            ...
            try:
                ...
                for sheet, data in df.items():
                    text = data.astype(str).values.flatten()
                    content = " ".join(text)
...
if __name__ == "__main__":
    excel_directory = r"C:\\Users\\91996\\OneDrive\\Desktop\\Incident_Chatbot\\prodsupp"
